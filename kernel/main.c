...
{
	return ((e->higher16 << 16) | e->lower16);
}
...
{
	e->higher16 = (unsigned short)((addr >> 16) & 0xFFFF);
	e->lower16 = (unsigned short)(addr & 0xFFFF);
}
...
static void stop_tracking_divbyzero(void)
{
	...
	if (orig_divbyzero != 0)
	{
		...
		_write_idt_entry(entry_pointer, orig_divbyzero);				
		orig_divbyzero = 0;		
	}
}
...
static void start_tracking_divbyzero(void)
{
	...
	{		
		...
		orig_divbyzero = _read_idt_entry(entry_pointer);
		_write_idt_entry(entry_pointer,(unsigned long)(__wrapper_divbyzero));		
	}
}
